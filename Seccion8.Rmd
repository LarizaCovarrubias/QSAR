...
En nuestro caso práctico, optamos por realizar el análisis y desarrollo del modelo con MLR directamente en RStudio, una plataforma ampliamente utilizada para estadísticas y análisis de datos. A continuación, presentaremos el procedimiento.

-   Generación modelo MLR

```{r}
# parsnip_addin()
# Crear el de modelo y motor
lm_model <- 
  linear_reg() %>% 
   set_engine("lm") 
...
rf_fit <- rf_wflow %>% fit(data = data_train)
```
...
Apliquemos el modelo de bosque aleatorio al conjunto de prueba:

```{r}
estimate_perf(rf_fit, data_test)
```
El modelo parece funcionar mucho mejor en los datos con los que se entrenó que en datos de prueba. 
...
Tampocoo debemos usar directamente los datos de prueba. Entonces, si no debemos usar directamente los datos de prueba ni repredecir las predicciones en el conjunto de entrenamiento, la solución está en utilizar métodos de remuestreo, como la validación cruzada o los conjuntos de validación. Estos métodos nos permiten obtener una evaluación más precisa y realista del rendimiento de nuestro modelo sin caer en la trampa de la sobreajuste que se produce al repredecir el conjunto de entrenamiento.
...
Estas partes son similares a los conjuntos de entrenamiento y prueba.
...
Alrededor de 25 muestras están en el conjunto de análisis y 13 están en ese conjunto de evaluación en particular.
...
Las estimaciones de desempeño son mas realistas que las estimaciones de resustitución.
